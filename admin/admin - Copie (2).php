									...
									if($tab_zone[$i] != "")
									{
										...
										if(sizeof($tab_medias[1]) > 0)	// si il y a des médias présents dans la zone
										{
											for($j = 0;$j<sizeof($tab_medias[1]);$j++) //[1] pour le résultat sans le séparateur [] et [0] avec séparateur
											{
												...
												if($ligne_medias->fichier_media != "") $new_media[$j]="<img id=\"im" . $ligne_medias->id_media . "\" src=\"../img/medias/media" . $ligne_medias->id_media . "." . $ligne_medias->fichier_media . "\" alt=\"" . $ligne_medias->alt_media . "\" />";
						...
						else
						{
							...
							if(in_array($extension, $allowedExts))
							{
								...
								else
								{
									...
									$chemin_media = "../img/medias/media" . $id_media_cree . "." . $extension;
...
***********************/
				case "supprimer_medias":
					...
					if(isset($_GET['cas']) && $_GET['cas'] == 2)   // si on a cliqué sur OUI
					{
						...
						if(isset($_GET['extension']) && $_GET['extension'] != "non") @unlink("../img/medias/media" . $_SESSION['id_media'] . "." . $_GET['extension']);	//on supprime le fichier media
...
***********************/
						...
						elseif($_FILES['fichier_media']['name'] != "")
						{
							...
							{
/*************************
	1 : il s'agit d'un fichier

	1.1 : on stocke le contenu des champs sauf la partie upload fichier
***********************/
								$extension = substr($fileName, strrpos($fileName, '.') + 1); // getting the info about the image to get its extension
...
/*************************
	1.2 : il s'agit d'une image
***********************/
								if(fichier_type($_FILES['fichier_media']['name']) != "pdf")
								{
									$chemin_media = "../img/medias/media" . $_SESSION['id_media'] . "." . $extension;
									if(is_uploaded_file($_FILES['fichier_media']['tmp_name']))
									{
										if(copy($_FILES['fichier_media']['tmp_name'], $chemin_media))
										{
/*************************
	1.2.1 : si l'upload s'est déroulé correctement, on stocke
***********************/
											if($extension != $_SESSION['fichier_media'])
											{
												$fichier_a_detruire = "../img/medias/media" . $_SESSION['id_media'] . "." . $_SESSION['fichier_media'];
...
***********************/
						...
						{
/*************************
	2 : Si l'on n'a pas fourni de fichier, on vérifie s'il s'agit d'une video ou d'une modification de fichier
***********************/
							if($_FILES['fichier_media']['name'] == "")
							{
/*************************
