...
***********************/
				case "pages":
									...
									if($tab_zone[$i] != "")
									{
										...
										if(sizeof($tab_medias[1]) > 0)	// si il y a des médias présents dans la zone
										{
											for($j = 0;$j<sizeof($tab_medias[1]);$j++) //[1] pour le résultat sans le séparateur [] et [0] avec séparateur
											{
												...
												if($ligne_medias->fichier_media != "") $new_media[$j]="<img id=\"im" . $ligne_medias->id_media . "\" src=\"../img/medias/media" . $ligne_medias->id_media . "_g." . $ligne_medias->fichier_media . "\" alt=\"" . $ligne_medias->alt_media . "\" />";
					...
					$requete_medias = "SELECT * FROM medias ORDER BY fichier_media, titre_media";	// toujours affichage des médias présents dans la table
...
***********************/
				case "modifier_pages":
					...
					$requete_affichage_medias = "SELECT * FROM medias ORDER BY fichier_media, titre_media";	// on affiche les médias présents dans la table
...
***********************/
				case "supprimer_pages":             
					...
					$requete_affichage_medias = "SELECT * FROM medias ORDER BY fichier_media, titre_media";	// on affiche toujours les medias présents dans la table
					$affichage_centre = afficher_medias($requete_affichage_medias, "centre", $connexion); 
...
***********************/
						...
						elseif($_FILES['fichier_media']['name'] == "" && empty($_POST['lien_media']))
						{
							$avertissement = "<label id=\"avertissement\">Collez une url ou sélectionnez un fichier</label>\n";
							$color_champ['lien_media'] = " class=\"color_champ\"";
							$color_champ['fichier_media'] = " class=\"color_champ\"";          
						}
						elseif($_FILES['fichier_media']['name'] != "" && (fichier_type($_FILES['fichier_media']['name']) == "jpg" || fichier_type($_FILES['fichier_media']['name']) == "png" || fichier_type($_FILES['fichier_media']['name']) == "gif")&& empty($_POST['alt_media']))
						{
							$avertissement = "<label id=\"avertissement\">Saisissez une légende pour cette image</label>\n";
							$color_champ['alt_media'] = " class=\"color_champ\"";        
						}          
/*************************
	tout est ok : on gère en fonction du cas
***********************/
						else
						{
/*************************
...
***********************/
							if($_FILES['fichier_media']['name'] == "")
							{
								...
								titre_media='" . addslashes($_POST['titre_media']) . "',
...
***********************/
							...
							{
								if(fichier_type($_FILES['fichier_media']['name']) == "jpg" ||
								fichier_type($_FILES['fichier_media']['name']) == "jpeg" ||  
								fichier_type($_FILES['fichier_media']['name']) == "png" ||  
								fichier_type($_FILES['fichier_media']['name']) == "gif" ||  
								fichier_type($_FILES['fichier_media']['name']) == "pdf" ||  
								fichier_type($_FILES['fichier_media']['name']) == "mp3")
								{
/*************************
	2.1 : on stocke le contenu des champs sauf la partie upload fichier
***********************/
									...
									$id_media_cree = $connexion->lastInsertId();
									$avertissement = "<label id=\"ok\">Insertion réussie</label>\n";
/*************************
	2.1.1 : il s'agit d'une image
***********************/
									if(fichier_type($_FILES['fichier_media']['name']) != "pdf" && fichier_type($_FILES['fichier_media']['name']) != "mp3")
									{
										$extension = fichier_type($_FILES['fichier_media']['name']);
										$chemin_media = "../img/medias/media" . $id_media_cree . "_g." . $extension;
										if(is_uploaded_file($_FILES['fichier_media']['tmp_name']))
										{
											if(copy($_FILES['fichier_media']['tmp_name'], $chemin_media))
											{
/*************************
	2.1.1.1 : si l'upload s'est déroulé correctement, on stocke
***********************/
												$requete_maj_medias = "UPDATE medias SET 
												fichier_media='" . $extension . "' 
												WHERE id_media='" . $id_media_cree . "'";
												$resultat_maj_medias = $connexion->exec($requete_maj_medias);
												$avertissement = "<label id=\"ok\">Le fichier a bien été chargé</label>\n";
											}
										}
									}							
/*************************
	2.1.2 : il s'agit d'un doc pdf
***********************/
									elseif(fichier_type($_FILES['fichier_media']['name']) == "pdf")
									{
										$chemin_media = "../img/medias/media" . $id_media_cree . ".pdf";
										if(is_uploaded_file($_FILES['fichier_media']['tmp_name']))
										{
/*************************
	2.1.2.1 : si l'upload s'est déroulé correctement, on stocke
***********************/
											if(copy($_FILES['fichier_media']['tmp_name'], $chemin_media))
											{
												$requete_maj_medias = "UPDATE medias SET 
...
/*************************
	2.1.3 : il s'agit d'un mp3
***********************/
									else
									{
										$chemin_media = "../img/medias/media" . $id_media_cree . ".mp3";
										if(is_uploaded_file($_FILES['fichier_media']['tmp_name']))
										{
												$_SESSION['toto'] = 'toto';
/*************************
	2.1.3.1 : si l'upload s'est déroulé correctement, on stocke
***********************/
											if(copy($_FILES['fichier_media']['tmp_name'], $chemin_media))
											{
												$requete_maj_medias = "UPDATE medias SET 
								...
								else $avertissement = "<label id=\"avertissement\">Seules les extensions png, gif, jpg et pdf sont autorisées</label>\n";
							}
						}
...
***********************/
				case "supprimer_medias":
					...
					if(isset($_GET['cas']) && $_GET['cas'] == 2)   // si on a cliqué sur OUI
					{
						...
						if(isset($_GET['extension']) && $_GET['extension'] != "non") @unlink("../img/medias/media" . $_SESSION['id_media'] . "_g." . $_GET['extension']);	//on supprime le fichier media
...
***********************/
						...
						{
/*************************
	1 : Si l'on n'a pas fourni de fichier, on vérifie s'il s'agit d'une video ou d'une modification de fichier
***********************/
							if($_FILES['fichier_media']['name'] == "")
							{
/*************************
	1.1 : pas d'url : il s'agit de renommer le media existant
***********************/
								if($_POST['lien_media'] == "")
								{
...
/*************************
	1.2 : on fournit une url : on passe d'un fichier à un lien ou on modifie un lien existant
***********************/
								else
								{
									if($_SESSION['fichier_media'] != "")
									{
										$fichier_a_detruire = "../img/medias/media" . $_SESSION['id_media'] . "_g." . $_SESSION['fichier_media'];
...
***********************/
									...
									$avertissement = "<label id=\"ok\">Insertion réussie</label>\n";
/*************************
	2.2 : il s'agit d'une image
***********************/
									if(fichier_type($_FILES['fichier_media']['name']) != "pdf")
									{
										$extension = fichier_type($_FILES['fichier_media']['name']);
										$chemin_media = "../img/medias/media" . $_SESSION['id_media'] . "_g." . $extension;
										if(is_uploaded_file($_FILES['fichier_media']['tmp_name']))
										{
											if(copy($_FILES['fichier_media']['tmp_name'], $chemin_media))
											{
/*************************
	2.2.1 : si l'upload s'est déroulé correctement, on stocke
***********************/
...
***********************/
									else
									{
										...
										$chemin_media = "../img/medias/media" . $_SESSION['id_media'] . ".pdf";
										if(is_uploaded_file($_FILES['fichier_media']['tmp_name']))
										{
											if(copy($_FILES['fichier_media']['tmp_name'], $chemin_media))
											{
												...
												$resultat_maj_medias = $connexion->exec($requete_maj_medias);
												$avertissement="<label id=\"ok\">Le fichier a bien été chargé</label>\n";
												if(fichier_type($_FILES['fichier_media']['name']) != $_SESSION['fichier_media']) unset($chemin_fichier);
											}
										}
									}                                
								}
					...
					$requete = "SELECT * FROM medias ORDER BY fichier_media, titre_media";	// on affiche toujours les medias présents dans la table
...
***********************/
				case "supprimer_actus":
					...
					$requete_affichage_medias = "SELECT * FROM medias ORDER BY fichier_media, titre_media";	// on affiche toujours les medias présents dans la table
					$affichage_centre = afficher_medias($requete_affichage_actus, "centre", $connexion);   
				...
				case "modifier_syndications":
					...
					if(isset($_POST['submit']))
					{
						...
						else
						{
							...
							$resultat_maj_syndications = $connexions->exec($requete_maj_syndications);
							$_SESSION['avertissement'] = "<label id=\"ok\">Modification effectuée.</label>";        
							header("Location:admin.php?action=syndications");
						}       
					}
...
***********************/
				case "evenements":
					...
					$requete4="SELECT * FROM medias ORDER BY fichier_media, titre_media";
					$affichage_centre=afficher_medias($requete4, "centre", $connexion);       
...
***********************/
				case "supprimer_evenements":
					...
					// on affiche toujours les medias présents dans la table
					$requete_affichage_medias = "SELECT * FROM medias ORDER BY fichier_media, titre_media";
					$affichage_centre = afficher_medias($requete_affichage_medias, "centre", $connexion);   
...
***********************/
				case "repondre_contact":
					...
					co.id_compte=ca.id_compte  
					ORDER BY ca.date_contact DESC";               
					$liste_messages = afficher_messages(requete_messages); 
...
***********************/
				case "css":
					...
					if(isset($_POST['submit']))
					{
						...
						{
							$resultat_insert_templates = $connexion->exec("INSERT INTO css SET id_template='" . $_POST['id_template']  . "', nom_css='" . remplacer_accents($_POST['nom_css']) . "'");
...
***********************/
				case "editer_css":
					...
					if(isset($_GET['id_css']))
					{
						...
						$fichier_texte = @fopen("../templates/" . $_SESSION['file_css'] . ".css", "r");
						if($fichier_texte)
						{
