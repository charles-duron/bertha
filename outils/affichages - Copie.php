	...
	function afficher_css($requete, $connexion)
	{
		...
		while($ligne = $sql->fetch())
		{
			...
			if($ligne->lien_css != "")
			{
				...
				$actions = "<td>     
				<a href=\"admin.php?action=editer_css&id_css=" . $ligne->id_css . "\"><img src=\"../img/icones/css.png\" title=\"éditer la feuille de style\" alt=\"\" /></a>
				...
				<a href=\"admin.php?action=supprimer_css&id_css=" . $ligne->id_css . "\"><img class=\"suprimer\" src=\"../img/icones/supprimer.png\" title=\"supprimer\" alt=\"\" /></a>
				</td>\n"; 
			}
			else
			{
			...
			$liste_css .= "<tr " . $style . ">\n";   
			$liste_css .= "<td style=\"text-align:left;padding-left:30px\"><img src=\"../img/icones/fleche_css.png\" alt=\"\" />" . $feuille_css . "</td>\n";  
			$liste_css .= $actions;		
			$liste_css .= "</tr>\n"; 
			$i++;
		}
...
***********************/
	function afficher_evenements($requete, $connexion)
	{
		...
		while($ligne = $sql->fetch())
		{
			...
			$liste_evenements .= "<td style=\"width:57%\"><h3>" . $ligne->titre_evenement . "</h3>" . substr($ligne->contenu_evenement,0,120) . "</td>\n";     
...
***********************/
	function afficher_templates($requete, $connexion)
	{
		...
		while($ligne = $sql->fetch())
		{
			...
			if($nb_template > 0) $css = "<img src=\"../img/icones/css.png\" alt=\"\" title=\"feuille(s) de style liée(s)\" />";
			else $css = "";
	...
	function afficher_medias($requete, $position, $connexion)
	{
		...
		switch($position)
		{
			case "centre":
				...
				while($ligne = $sql->fetch())
				{
					if($ligne->lien_media != "") $cas = 1;	// cas des vidéos
					else
					{
						if($ligne->fichier_media != "pdf") $cas = 2;	//cas des images
						else$cas = 3;	//cas des doc pdf
					}
					$tab_lien[$i] = $cas;
					if($i>0 && $tab_lien[$i] != $tab_lien[$i-1]) $liste_medias .= "<hr />\n";
/*************************
	1 : il s'agit de fichier image ou pdf
***********************/
					if($ligne->fichier_media != "" &&  $ligne->fichier_media != "mp3")
					{
						if($ligne->fichier_media != "pdf")// cas des images
						{         
							$image = "../img/medias/media" . $ligne->id_media . "_g." . $ligne->fichier_media;
							$code_css = "img" . $ligne->id_media;
							$lien = $image;
						}
						else //cas des pdf
						{
							...
							$lien = "../img/medias/media" . $ligne->id_media . ".pdf";
						}
						$code_insertion = "[" . $ligne->id_media . "]";  
						$liste_medias .= "<div style=\"background-image:url(" . $image . "?" . code_image(4) . ")
						\">\n<img src=\"" . $image . "\" alt=\"\" />\n";      
						$liste_medias .= "<p title=\"id=" . $code_css . "\">" . $code_insertion . "</p>\n</div>\n";          
					}        
/*************************
	2 : il s'agit d'un fichier mp3
***********************/
					else if($ligne->fichier_media == "mp3")
					{
						...
						$liste_medias .= "<audio src=\"../img/medias/media" . $ligne->id_media . ".mp3\" preload=\"auto\" />";                      
						$liste_medias .= "<p title=\"id=" . $code_css . "\">" . $media . "</p>\n</div>\n";                    
					}
/*************************
	2 : il s'agit d'une video
***********************/
					else
					{
						$liste_medias .= "<div>\n";
			...
			case "droite":
				...
				while($ligne = $sql->fetch())
				{
					...
					if($ligne->fichier_media != "")
					{
						if($ligne->fichier_media != "pdf")// cas des images
						{
							$media = "<img class=\"apercu_image\" src=\"../img/medias/media" . $ligne->id_media . "_g." . $ligne->fichier_media ."\" alt=\"\" />";
						...
						{
							$media = "<a href=\"../img/medias/media" . $ligne->id_media . ".pdf\" target=\"_blank\"><img class=\"apercu_image\" src=\"../img/icones/pdf.png\" alt=\"Document pdf\" /></a>";          
							$slide = "NC";
							$press_book = "NC";
						}
					...
					{
						$media = "<a href=\"../img/medias/media" . $ligne->id_media . ".mp3\" target=\"_blank\"><img class=\"apercu_image\" src=\"../img/icones/music.png\" alt=\"Morceau de musique\" /></a>";          
						$slide = "NC";
						$press_book = "NC";
					}
					else //cas des vidéos
