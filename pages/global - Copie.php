...
***********************/
	...
	while($ligne = $sql->fetch())
	{
		if($i == 0)
		{
			...
			if($ligne->calendrier == "oui") $calendrier = "calendrier.php";
		...
		$_SESSION['style'] .= "<link rel=\"stylesheet\" type=\"text/css\"  href=\"../templates/" . $ligne->lien_css . ".css\"/>\n";
		$_SESSION['style_admin'] = "<link rel=\"stylesheet\" type=\"text/css\" href=\"../css/admin" . $ligne->id_theme . ".css\" />\n";      
		$i++;
	}       
/*************************
	construction du menu des langues (uniuement s'il y a plus de 2 langues)
***********************/
...
***********************/
	...
	while($ligne = $sql->fetch())
	{
		...
		$requete = "SELECT count(*) FROM pages WHERE id_rubrique='" . $ligne->id_rubrique . "'";
		$sql_count = $connexion->prepare($requete);
		$sql_count->execute();
		$nb = $sql_count->fetchColumn();
		if($nb > 0)
		{
			$requete = "SELECT * FROM pages WHERE id_rubrique='" . $ligne->id_rubrique . "' ORDER BY rang";
...
***********************/
	...
	while($ligne = $sql->fetch())
	{
		$slider .= "<li><a href=\"javascript:void(0)\"><img src=\"../img/medias/media" . $ligne->id_media . "_g." . $ligne->fichier_media . "\" /><p>" . $ligne->alt_media . "</p></a></li>\n";
	}
	$slider .= "</ul>\n";
/*************************
...
***********************/
	if(isset($tab_actus))
	{
		...
		for($i = 0; $i < sizeof($tab_actus); $i++)
		{
			...
			{
				if($i == 0) $requete .= "('" . $tab_actus[$i] . "'";
				else $requete .= ",'" . $tab_actus[$i] . "'";
			}
		}
...
***********************/
					for($j = 0; $j < sizeof($tab_medias[1]); $j++) //[1] pour le résultat sans le séparateur [] et [0] avec séparateur
					{
						...
						{
							$new_media[$j] = "<p id=\"pdf" . $ligne_media->id_media . "\"><a href=\"../img/medias/media" . $ligne_media->id_media . "." . $ligne_media->fichier_media . "\" target=\"_blank\">" . $ligne->titre_actu . "</a></p>\n";
						}
						if($ligne_media->fichier_media != "pdf")
						{
							$new_media[$j] = "<img id=\"img" . $ligne_media->id_media . "\" src=\"../img/medias/media" . $ligne_media->id_media . "." . $ligne_media->fichier_media . "\" alt=\"" . $ligne_media->alt_media . "\" />\n";
						}              
...
***********************/
			...
			case "galerie_photos":
				...
				while($ligne = $sql->fetch())
				{
					...
					$g_image = "../img/medias/media" . $ligne->id_media . "_g." . $ligne->fichier_media;
					$p_image = "../img/medias/media" . $ligne->id_media . "_p." . $ligne->fichier_media;
					$contenu .= "<div style=\"background-image:url(../img/medias/media" . $ligne->id_media . "_g." . $ligne->fichier_media . ")\">\n<a id=\"a_fancy\" rel=\"group\" href=\"" . $g_image . "\" title=\"" . $legende . "\">" . $legende . "\n</a>\n</div>\n";
				}
				$contenu .= "</div>\n";
