...
***********************/
	...
	for ($jour = 1; $jour <= $dernierJour; $jour++)
	{
		...
		$cptJour++;
/*************************
	1 On vérifie si un évènement à lieu ce jour-là
***********************/
		...
		$nb = $sql->fetchColumn();
/*************************
	1.1 Un événement a bien lieu. On vérifie qu'il n'y en a qu'un seul
***********************/
		if($nb != 0)
		{
/*************************
	1.1.1 S'il y a 2 évènements (limite permise par le calendrier)
***********************/
			if($nb > 1)
			{
				$requete = "SELECT * FROM evenements WHERE (date_debut_evenement<='" . $annee . "-" . $mois . "-" . $jour . "' AND date_fin_evenement>='" . $annee . "-" . $mois . "-" . $jour . "') AND id_langue='" . $_SESSION['langue'] . "' AND visible='oui' ORDER BY date_debut_evenement";
				$sql = $connexion->query($requete);
				$sql->setFetchMode(PDO::FETCH_OBJ);
/*************************
	1.1.1.1 On ajoute la classe permettant un affichage correct de la date sur 2 liens
***********************/
				if($classe == "") $classe = "class=\"double_event\"";
				else
				{
					$inner_class = explode("\"", $classe);
					$classe = "class=\"" . $inner_class[1] . " double_event\"";
				}
				
				$calEven .= "<td " . $classe . ">";
/*************************
	1.1.1.2 Création des 2 liens
***********************/
				for($i = 0; $ligne = $sql->fetch(); $i++)
				{
					if($i == 0)
					{
						if($ligne->nature != "") $style = " style=\"background-color:#" . $ligne->nature . "; color:#" . $ligne->nature . "\"";
						else $style = "";
						$span = "";
					}
					else
					{
						...
						$span = "\n<span class=\"span_double_event\" style=\"color:#" . $ligne->nature . "\" >" . $jour . "</span>";
					}

					$calEven .= "<a href=\"../pages/global.php?action=calendrier&amp;id_evenement=" . $ligne->id_evenement . "\"" . $style . " data-tip=\"" . $ligne->titre_evenement . "\" ><span>" . $jour . "</span></a>" . $span;
				}
				
				$calEven .= "</td>\n";
			}
/*************************
	1.1.2 Il n'y en a qu'un
***********************/
			else
			{
				...
				$calEven .= "<td " . $classe . "><a href=\"../pages/global.php?action=calendrier&amp;id_evenement=" . $ligne->id_evenement . "\"" . $style . " data-tip=\"" . $ligne->titre_evenement . "\"><span>" . $jour . "</span></a></td>\n";
			}
		}
/*************************
	1.2 Aucun évènement ce jour-là
***********************/
		else $calEven .= "<td " . $classe . ">" . $jour . "</td>\n";   
/*************************
	En fin de semaine, on ferme la rangée et on en rouvre une si il y a une semaine suivante
***********************/
